...
echo '<div class="nectar_cascading_images" data-border-radius="'.esc_attr($cascading_attrs['border_radius']).'" data-animation-timing="'.esc_attr($cascading_attrs['animation_timing']).'">';
...
for( $i = 1; $i < 5; $i++ ){
	...
	$img_markup        = (!empty($image_url)) ? '<div style=" -webkit-transform:'.$transform_string.'; -ms-transform:'.$transform_string.'; transform:'.$transform_string.';" class="img-wrap"> <img src="'.$image_url.'" class="skip-lazy" alt="'.esc_attr($image_alt).'" /> </div>': null;
	...
	if( !empty($image_url) || $data_has_bg_color === 'true' ) {
		echo '<div class="cascading-image" data-has-img="'.esc_attr($data_has_bg_img).'" style=" padding:'.$transform_arr .'%;" data-animation="'.strtolower(esc_attr($parsed_animation)).'" data-shadow="'.esc_attr($cascading_attrs['image_'.$i.'_box_shadow']).'"><div class="inner-wrap">'.$bg_color_markup . $img_markup.'</div></div>';
	}
