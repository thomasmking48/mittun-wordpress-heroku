...
// Output.
$output .= "\n\t".'<div '.$style.' class="'.esc_attr($css_class).'" '.$using_bg . $column_data_attrs . '>' . $column_link_html . $border_html . $image_bg_markup . $video_markup;
$output .= '<div class="column-bg-overlay"'.$column_overlay_style.'></div>';

if( $using_reveal_animation === true ) { 
  $output .= "\n\t\t".'<div class="column-inner-wrap"><div '.$style2.' data-bg-cover="'.esc_attr($enable_bg_scale).'" class="column-inner '.$column_padding.'">'; 
}
else { 
