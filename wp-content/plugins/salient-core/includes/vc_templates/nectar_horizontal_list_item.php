...
echo '<div class="nectar-hor-list-item '.$hasbtn_class.'" data-font-family="'.esc_attr($font_family).'" data-color="'.esc_attr($hover_color).'" data-columns="'.esc_attr($columns).'" data-column-layout="'.esc_attr($column_layout_to_use).'">'; 
	...
	for($i = 0; $i < intval($columns); $i++) {
		...
		echo '<div class="nectar-list-item" data-text-align="'.esc_attr($atts['col_'.$index_to_grab.'_text_align']).'">'. wp_kses_post($atts['col_'.$index_to_grab.'_content']) . $cta_1_markup . $cta_2_markup. '</div>';
	}
