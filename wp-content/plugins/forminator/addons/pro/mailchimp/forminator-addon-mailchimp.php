	...
	public function configure_api_key( $submitted_data, $form_id = 0 ) {
		...
		if ( isset( $submitted_data['api_key'] ) ) {
			...
			if ( ! $api_key_validated ) {
				...
				if ( $show_success ) {
					...
					return array(
						'html'         => '<div class="integration-header"><h3 class="sui-box-title" id="dialogTitle2">' . sprintf( __( '%1$s Added', Forminator::DOMAIN ), 'MailChimp' ) . '</h3></div>
		...
		return array(
			'html'       => '<div class="integration-header">
					<h3 class="sui-box-title" id="dialogTitle2">' . sprintf( __( 'Configure %1$s', Forminator::DOMAIN ), 'MailChimp' ) . '</h3>
					' . $this->settings_help() . '
					' . $error_message . '
				</div>
				<form>
					<div class="sui-form-field ' . ( ! empty( $api_key_error_message ) ? 'sui-form-field-error' : '' ) . '">
						...
						<div class="sui-control-with-icon">
							<input name="api_key"
								placeholder="' . sprintf( __( 'Enter %1$s API Key', Forminator::DOMAIN ), 'MailChimp' ) . '"
