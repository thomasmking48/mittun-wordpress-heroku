		...
		$html .= '<div class="forminator-timepicker">';
			...
			$html .= '<div class="forminator-row" data-multiple="true">';
				...
				if ( 'twelve' === $type ) {
					...
					$ampm = array(
						'name'  => $id . '-ampm',
						'id'    => 'forminator-field-' . $id . '-meridian',
						'class' => 'forminator-select',
					);
	...
	public function get_validation_messages() {
		...
		if ( $this->is_required( $field ) ) {
			...
			$hours_message = apply_filters(
				...
				( ! empty( $required_message ) ? '<strong>' . $hours_label . '</strong>: ' . $required_message : __( 'This field is required. Please input a valid hour', Forminator::DOMAIN ) ),
				$id,
				$field
			);
		...
		if ( $this->is_required( $field ) ) {
			...
			$minutes_message = apply_filters(
				...
				( ! empty( $required_message ) ? '<strong>' . $minutes_label . '</strong>: ' . $required_message : __( 'This field is required. Please input a valid minute', Forminator::DOMAIN ) ),
				$id,
				$field
			);
