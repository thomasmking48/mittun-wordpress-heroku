...
abstract class Forminator_Result {
	...
	public function add_rewrite_rules() {
		...
		add_rewrite_rule( '([0-9]{4})/([0-9]{1,2})/([^/]+)/entries/?([0-9]{1,})/?$', 'index.php?year=$matches[1]&monthnum=$matches[2]&name=$matches[3]&entries=$matches[4]', 'top' );

		add_rewrite_rule( '(.?.+?)/entries/?([0-9]{1,})/?$', 'index.php?name=$matches[1]&entries=$matches[2]', 'top' );
