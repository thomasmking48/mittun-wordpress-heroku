 ...
 * @return mixed
 */
function forminator_get_stripe_vars() {
	$vars_list = array(
		'stripe_payment_mode'     => esc_html__( 'Payment Mode', Forminator::DOMAIN ),
		'stripe_payment_status'   => esc_html__( 'Payment Status', Forminator::DOMAIN ),
		'stripe_payment_amount'   => esc_html__( 'Payment Amount', Forminator::DOMAIN ),
		'stripe_payment_currency' => esc_html__( 'Payment Currency', Forminator::DOMAIN ),
		'stripe_transaction_id'   => esc_html__( 'Transaction ID', Forminator::DOMAIN ),
	);
...
function forminator_replace_form_data( $content, $data, Forminator_Custom_Form_Model $custom_form = null, Forminator_Form_Entry_Model $entry = null ) {
	...
	$content = forminator_replace_form_stripe_data( $content, $custom_form, $entry );
		...
		foreach ( $matches[0] as $match ) {
			...
			if ( isset( $data[ $element_id ] ) && ! empty( $data[ $element_id ] ) ) {
				$value = $data[ $element_id ];
			             ...
			             || strpos( $element_id, 'stripe' ) !== false )
			           && $custom_form && $entry ) {
...
function render_entry( $item, $column_name, $field = null ) {
	...
	$is_stripe = false;
	if ( stripos( $column_name, 'stripe' ) !== false ) {
		$is_stripe = true;
	}

	if ( $is_stripe ) {
		return Forminator_Form_Entry_Model::meta_value_to_string( 'stripe', $data, true );
	}
 ...
 * @return mixed
 */
function forminator_replace_form_stripe_data( $content, Forminator_Custom_Form_Model $custom_form = null, Forminator_Form_Entry_Model $entry = null ) {
	...
	if ( ! empty( $form_fields ) ) {
		foreach ( $form_fields as $field ) {
			...
			if ( 'stripe' === $field_type ) {
				$stripe_slug = $field->slug;
			}
		}
	}
	if ( ! empty( $stripe_slug ) && ! empty( $entry ) ) {
		$stripe_meta = isset( $entry->meta_data[ $stripe_slug ] ) ? $entry->meta_data[ $stripe_slug ]['value'] : array();
		if ( ! empty( $stripe_meta ) ) {
			if ( strpos( $content, '{stripe_payment_mode}' ) !== false ) {
				$content = str_replace( '{stripe_payment_mode}', $stripe_meta['mode'], $content );
			}
			if ( strpos( $content, '{stripe_payment_status}' ) !== false ) {
				$content = str_replace( '{stripe_payment_status}', $stripe_meta['status'], $content );
			}
			if ( strpos( $content, '{stripe_payment_amount}' ) !== false ) {
				$content = str_replace( '{stripe_payment_amount}', $stripe_meta['amount'], $content );
			}
			if ( strpos( $content, '{stripe_payment_currency}' ) !== false ) {
				$content = str_replace( '{stripe_payment_currency}', $stripe_meta['currency'], $content );
			}
			if ( strpos( $content, '{stripe_transaction_id}' ) !== false ) {
				$content = str_replace( '{stripe_transaction_id}', $stripe_meta['transaction_id'], $content );
			}
		}
	}

	return apply_filters( 'forminator_replace_form_stripe_data', $content, $custom_form, $entry );
}
