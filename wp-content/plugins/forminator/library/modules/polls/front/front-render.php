...
class Forminator_Poll_Front extends Forminator_Render_Form {
	...
	public function render_form_header() {
		...
		if ( isset( $_REQUEST['saved'] ) && ! isset( $_REQUEST['results'] ) ) { // WPCS: CSRF OK
			...
			if ( ! $this->is_preview && ! $this->model->current_user_can_vote() && ! isset( $_REQUEST['action'] ) ) {
				$this->track_views = false;
				?>
	...
	public function success_footer_script( $model, $container_id ) {
			...
			(function ($, doc) {
				...
				jQuery('document').ready(function () {
					...
					function drawPollResults_<?php echo esc_attr( $container_id ); ?>() {
						var data = google.visualization.arrayToDataTable([
							...
							if ( ! is_null( $fields ) ) {
								...
								foreach ( $fields as $field ) {
									$annotation = '';
									$label      = 'bar' === $chart_design && ( strlen( $field->title ) > 9 ) ? addslashes( substr( $field->title, 0, 6 ) . '...' ) : addslashes( $field->title );
									$tooltip[]  = addslashes( $field->title );
						...
						<?php if ( 'bar' === $chart_design ) { ?>
						google.visualization.events.addListener(chart, 'onmouseover', function(e) {
							setTooltipContent(<?php echo json_encode( $tooltip ); ?>, data, e.row);
						});
						<?php } ?>
					}
					function setTooltipContent(tooltip, data, row) {
						if (row != null) {
							var content = '<div class="custom-tooltip" ><strong>' + tooltip[row] + '</strong><div class="tooltip-result"><?php _e( 'vote(s)', Forminator::DOMAIN ); ?>: <strong>' + data.getValue(row, 1) + '</strong></div></div>',
								element = document.getElementsByClassName("google-visualization-tooltip")[0];
							if( element ){
		...
		<style type="text/css">
			.google-visualization-tooltip{width: auto !important; height: auto !important; padding: 10px;}
		</style>
		<?php
	}
