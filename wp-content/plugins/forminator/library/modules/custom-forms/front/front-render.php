...
class Forminator_CForm_Front extends Forminator_Render_Form {
	...
	public function is_only_hidden( $wrapper ) {
		...
		if ( ! isset( $wrapper['fields'] ) ) return false;

		// We have more than one field in the row, abort
		if ( count( $wrapper['fields'] ) > 1 ) return false;

		// Check if the field type is hidden
		if( "hidden" === $wrapper['fields'][0]['type'] || "stripe" === $wrapper['fields'][0]['type'] ) {
		...
		foreach ( $properties as $property => $value ) {
			...
			foreach ( $form_fields as $form_field ) {
				if ( $this->is_pagination( $form_field ) ) {
					...
					$properties[ $element ]['pagination-labels'] = isset( $form_field['pagination-labels'] ) ? $form_field['pagination-labels'] : '';
					$properties[ $element ]['prev-text']         = isset( $form_field['pagination-footer-button-text'] ) ? $form_field['pagination-footer-button-text'] : '';
					$properties[ $element ]['next-text']         = isset( $form_field['pagination-right-button-text'] ) ? $form_field['pagination-right-button-text'] : '';
				}
			}
		}
