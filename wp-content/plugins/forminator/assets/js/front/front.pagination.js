...
(function ($, window, document, undefined) {
	...
	$.extend(ForminatorFrontPagination.prototype, {
		...
		render_footer_navigation: function() {
			if ( this.custom_label[this.element] && this.custom_label[this.element]['pagination-labels'] === 'custom'){
		...
		update_buttons: function () {
			if (this.step === 0) {
				this.$el.find('.forminator-button-back').attr('disabled', true);
			} else {
				this.$el.find('.forminator-button-back').removeAttr('disabled');
			}
			...
			if ( this.step === ( this.totalSteps - 1 ) ) {
				...
				if ( this.$el.hasClass('forminator-design--material') ) {
					...
					this.$el.find('.forminator-button-back .forminator-button--text').html( this.prev_button );
					...
					this.$el.find('.forminator-button-back').html( this.prev_button );
